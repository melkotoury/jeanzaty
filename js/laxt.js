...
$(function() {
    ...
    $(document).on("ready", function() {
        ...
        $(".countdown").countdown({
            until: new Date(
                Date.parse(setting.counter.lastDate)
            ),
    ...
    // 7.2. countdown SETUP
    var setting = { counter: { lastDate: "12/26/2018 12:00:00", timeZone: null } }; // target date settings, 'MM/DD/YYYY HH:MM:SS'
    ...
    $("form#form").on("submit", function() {
        ...
        if (($(".requiredField").each(function() {
                if ("" === jQuery.trim($(this).val())) $(this)
                    .prev("label")
                    .text(), $(this)
                    .parent()
                    .append('<span class="error">This field is required</span>'), $(this).addClass("inputError"), (s = !0);
                else if ($(this).hasClass("email")) {
                    var r = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
                    r.test(jQuery.trim($(this).val())) || ($(this)
                        .prev("label")
                        .text(), $(this)
                        .parent()
                        .append('<span class="error">Invalid email address</span>'), $(this).addClass("inputError"), (s = !0));
                }
            }), !s)) {
            $("form#form input.submit").fadeOut(
                "normal",
                function() {
                    $(this)
                        .parent()
    ...
    $("form#subscribe").on("submit", function() {
        ...
        var s = !1;
        if (($(".subscribe-requiredField").each(
                function() {
            ...
            $("form#subscribe input.submit").fadeOut(
                "normal",
                function() {
                    $(this)
                        .parent()
    ...
    // 9. modals
    // 9.1. sign up modal
    $(".sign-up-modal-launcher, .sign-up-modal-closer").on(
        "click",
        function() {
    ...
    // 9.1.1. sign up modal additional CLOSER
    $(".header-navigation a, .header-navigation-xs a").on(
        "click",
        function() {
            $(".sign-up-modal").removeClass("open");
            $(".sign-up-modal").addClass("close");
    ...
    // 9.2. contact modal
    $(".contact-modal-launcher, .contact-modal-closer").on(
        "click",
        function() {
